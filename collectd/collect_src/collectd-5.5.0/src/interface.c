...
#  error "No suitable type for `struct ifaddrs->ifa_data' found."
	...
	struct IFA_DATA *if_data;
        derive_t rx_total_octets = 0;
        derive_t tx_total_octets = 0;
        int interface_data_found = 0;
	...
	for (if_ptr = if_list; if_ptr != NULL; if_ptr = if_ptr->ifa_next)
	{
		if (if_ptr->ifa_addr != NULL && if_ptr->ifa_addr->sa_family == AF_LINK) {
                    if( (strncmp(if_ptr->ifa_name, "lo", 2) == 0) || 
                        (strncmp(if_ptr->ifa_name, "loopback", 8) == 0) )
                        continue;
			...
			if_submit (if_ptr->ifa_name, "if_errors",
				...
				if_data->IFA_TX_ERROR);
                        ...
                        rx_total_octets= rx_total_octets + (derive_t)if_data->IFA_RX_BYTES;
                        tx_total_octets= tx_total_octets + (derive_t)if_data->IFA_TX_BYTES;
                        interface_data_found = 1;
		}
	}

	if(interface_data_found)
		if_submit ("all-interfaces", "if_octets",rx_total_octets, tx_total_octets);
...
/* #endif HAVE_GETIFADDRS */
	...
	while (fgets (buffer, 1024, fh) != NULL)
	{
		...
		if( (strncmp(device, "lo", 2) == 0) ||
                    (strncmp(device, "loopback", 8) == 0) )
                        continue;
		
		incoming = atoll (fields[0]);
		outgoing = atoll (fields[8]);
		if_submit (device, "if_octets", incoming, outgoing);
                rx_total_octets = rx_total_octets + incoming;
                tx_total_octets = tx_total_octets + outgoing;
		interface_data_found = 1;
	...
	for (i = 0; i < numif; i++)
	{
		if (kstat_read (kc, ksp[i], NULL) == -1)
			continue;
                ...
                if( (strncmp(ksp[i]->ks_name, "lo", 2) == 0) ||
                    (strncmp(ksp[i]->ks_name, "loopback", 8) == 0) )
                        continue;
		...
		rx_total_octets = rx_total_octets + (derive_t)rx;
		tx_total_octets = tx_total_octets + (derive_t)tx;
		interface_data_found = 1;
...
/* #endif HAVE_LIBKSTAT */
	...
	for (i = 0; i < num; i++)
        {
		if( (strncmp(ios[i].interface_name, "lo", 2) == 0) ||
                    (strncmp(ios[i].interface_name, "loopback", 8) == 0) )
                        continue;

		if_submit (ios[i].interface_name, "if_octets", ios[i].rx, ios[i].tx);
		rx_total_octets = rx_total_octets + (derive_t)ios[i].rx;
                tx_total_octets = tx_total_octets + (derive_t)ios[i].tx;
                interface_data_found = 1;
	}
	if (interface_data_found)
                if_submit ("all-interfaces", "if_octets", rx_total_octets, tx_total_octets);
...
/* #endif HAVE_LIBSTATGRAB */
	...
	for (i = 0; i < ifs; i++)
	{
		if( (strncmp(ifstat[i].name, "lo", 2) == 0) ||
                    (strncmp(ifstat[i].name, "loopback", 8) == 0) )
                        continue;
		...
		if_submit (ifstat[i].name, "if_errors", ifstat[i].ierrors, ifstat[i].oerrors );
                rx_total_octets = rx_total_octets + (derive_t)ifstat[i].ibytes;
                tx_total_octets = tx_total_octets + (derive_t)ifstat[i].obytes;
                interface_data_found = 1;
	}
        if (interface_data_found)
                if_submit ("all-interfaces", "if_octets", rx_total_octets, tx_total_octets);
